...
function initCube(x, y, z) {
  ...
  if (
    cubeGroup.children.some(
      (cube) =>
        distance(cube, { position: { x: x, y: y, z: z } }) < boxDimensions
    )
  )
    return;
...
function loop() {
  ...
  if (counter >= 10) {
    ...
    var xcube = cubeGroup.children[random(0, cubeGroup.children.length - 1)];
    xcube.reverseX = xcube.reverseX == true ? false : true;
    var ycube = cubeGroup.children[random(0, cubeGroup.children.length - 1)];
    ycube.reverseY = ycube.reverseY == true ? false : true;
    var zcube = cubeGroup.children[random(0, cubeGroup.children.length - 1)];
